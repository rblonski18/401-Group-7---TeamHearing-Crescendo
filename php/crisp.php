...
switch($_SERVER['REQUEST_METHOD']){
	case "GET":
		$query = $conn->prepare(
			...
			FROM CRISP 
			INNER JOIN Profiles p1 on CRISP.subuser = p1.ID 
			INNER JOIN Profiles p2 on CRISP.user = p2.ID 
			WHERE subuser = ?"
		);
	...
	case "POST":
		...
		$query = $conn->prepare(
			"INSERT INTO CRISP (behavior,calls,ear,gain,noise,notes,practice,responses,score,series,setting,snr,subuser,user) 
			VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
		);
		$query->bind_param('isidisisdsidii',
			$behavior,
			$_POST['calls'],
			$ear,
			$_POST['gain'],
