<?php
include_once 'utilities.php';
switch($_SERVER['REQUEST_METHOD']){
	case "GET":
		$query = $conn->prepare(
			"SELECT Gabor.*,
			CONCAT(p1.firstname,' ',p1.lastname) AS subuser, 
			CONCAT(p2.firstname,' ',p2.lastname) AS user
			FROM Gabor 
			INNER JOIN Profiles p1 on Gabor.subuser = p1.ID 
			INNER JOIN Profiles p2 on Gabor.user = p2.ID 
			WHERE subuser = ?"
		);
		$query->bind_param('i',$_GET['subuser']);
		$query->execute();
		echo json_encode(get_result($query)); break;
	case "POST":
		$behavior = convert2ID('Behavior','behavior',$_POST['behavior']);
		$ear = convert2ID('Ear','ear',$_POST['ear']);
		$noise = convert2ID('Noise','noise',$_POST['noise']);
		$practice = convert2ID('Switch','switch',$_POST['practice']);
		
		$query = $conn->prepare(
			"INSERT INTO Gabor (calls, mode, notes, responses, series, subuser, user) 
			VALUES (?,?,?,?,?,?,?)"
		);
		$query->bind_param('sisssii',
			$_POST['calls'],
			$_POST['mode'],
			$_POST['notes'],
			$_POST['responses'],
			$_POST['series'],
			$_POST['subuser'],
			$_POST['user']
		);
		$query->execute();
		$query->close();
		echo json_encode($conn->insert_id);
		
		// post to Activity
		postToActivity('Gabor',$_POST['subuser'],$_POST['user']);
}
?>