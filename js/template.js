...
Template.prototype.response = function (button) {
	var that = this;//extended scope
...
Template.prototype.stimulus = function () {
	var that = this;//extended scope
	processor.signal(this.path+this.note+'_'+this.instrument+'.mp3');
	var secondnote = this.note+this.spacing;
	console.log(this.path+secondnote+'_'+this.instrument+'.mp3');
	setTimeout(function(){processor.signal(that.path+secondnote+'_'+that.instrument+'.mp3');},1e3);
};
Template.prototype.test = function () {
	...
	// message
	layout.message('Template','Start Message',{
		Start: function () {
